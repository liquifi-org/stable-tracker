/**
 * Sync country remittances outflows from the World Bank.
 *   BM.TRF.PWKR.CD.DT — Personal remittances, paid (current USD)
...
async function fetchRemittances(): Promise<WorldBankEntry[]> {
...
export async function run(): Promise<void> {
    ...
    console.log('Fetching remittances outflows from the World Bank...');

    const entries = await fetchRemittances();
    ...
    for (const entry of entries) {
        ...
        ops.push({
            updateOne: {
                ...
                update: {
                    $set: {
                        remittancesSent: entry.value,
                        remittancesYear: Number(entry.date),
    ...
    console.log(`Countries with remittances data: ${updated}`);
